...
const theme = {
  ...
  'family:text': {
    ...
    input:
      'w-full px-3 py-2 transition-all border rounded-lg border-x11 focus:border-crayola focus:border-2 focus:shadow-input placeholder-x11 text-sm'
  },
  'family:date': {
    inner:
      'flex items-center max-w-md ring-1 ring-gray-400 focus-within:ring-crayola focus-within:ring-2 [&>label:first-child]:focus-within:text-crayola rounded-lg mb-1',
  ...
  color: {
    ...
    input:
      '$reset appearance-none w-full cursor-pointer border-0 rounded p-0 m-0 bg-transparent [&::-webkit-color-swatch-wrapper]:p-0 [&::-webkit-color-swatch]:border-0',
  ...
  slider: {
    ...
    help: 'mt-0 mb-1',
    sliderInner:
      'flex items-center py-1 [&>.formkit-max-value]:mb-0 [&>.formkit-max-value]:ml-8 [&>.formkit-max-value]:shrink [&>.formkit-max-value]:grow-0 [&>.formkit-icon]:bg-none [&>.formkit-icon]:border-0 [&>.formkit-icon]:p-0 [&>.formkit-icon]:w-4 [&>.formkit-prefix-icon]:mr-2 [&>.formkit-suffix-icon]:ml-2 [&[data-has-mark-labels="true"]_.formkit-track]:mb-4',
